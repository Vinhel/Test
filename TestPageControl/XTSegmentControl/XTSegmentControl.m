...
//                    [_titleIconView sd_setImageWithURL:[title urlImageWithCodePathResizeToView:_titleIconView] placeholderImage:kPlaceholderMonkeyRoundView(_titleIconView)];
                    _titleIconView.image = [UIImage imageNamed:title];
                }else{
...
//        全部任务的frame
        CGRect firstFrame = CGRectMake(0, 0, self.frame.size.width/4-1, height);
                ...
                [UIView animateWithDuration:XTSegmentControlAnimationTime animations:^{
                    ...
                    [_items enumerateObjectsUsingBlock:^(XTSegmentControlItem *item, NSUInteger idx, BOOL *stop) {
                        ...
                        if (index == idx) {
                            ...
                            imgName = [name replaceStr:@"off" withString:@"on"];

                        }else if(weakSelf.currentIndex == idx) {
                            NSString *name = [weakSelf.titleMutArr objectAtIndex:idx];
                            imgName = [name replaceStr:@"on" withString:@"off"];
                        }
                        if (imgName) {
